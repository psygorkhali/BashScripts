...
######################################################################
# Script : f_points_cus_yr_history_ld.ksh
...
GET_HALFYR_SEG_SQL="
SELECT 'BETWEEN.'''||TRIM(YR_START_DT)||'''.AND.'''||TRIM(YR_END_DT)||''''
||'|'||'BETWEEN.'''||TRIM(MIN_YR_KEY)||'''.AND.'''||TRIM(MAX_YR_KEY)||''''
FROM (
	SELECT  YR_START_DT
		,CASE WHEN CURRENT_DATE<YR_END_DT THEN CURRENT_DATE ELSE YR_END_DT END YR_END_DT
		,MIN(YR_KEY) MIN_YR_KEY
		,MAX(YR_KEY) MAX_YR_KEY
...
GET_YR_SEG=$(get_result -d "${VIEW_DB}" -q "$GET_HALFYR_SEG_SQL" -m "Unable to get HALFYR Seg")
print_msg "The halfyr segments are ${GET_YR_SEG}"
export GET_YR_SEG

for YR_SEG in $GET_YR_SEG
do

	IFS="|"
	set -A ARR_YR_SEG ${YR_SEG}	  
	YRDT_CLAUSE=$(echo ${ARR_YR_SEG[0]}|tr -t "." " ")
	YR_CLAUSE=$(echo ${ARR_YR_SEG[1]} | tr "." " ")
	
	print_msg "YEAR CLAUSE:=${YRDT_CLAUSE}"
	print_msg "YR KEY CLAUSE:=${YR_CLAUSE}"
