...
######################################################################
# Script : f_points_cus_wk_ld.ksh
# Description : This script loads the Points data
#               table DWH_F_RWD_CHOICE_CUS_WK_A using SOURCE TABLE 
...
TARGET_TABLE="DWH_F_RWD_CHOICE_CUS_WK_A"
TEMP_TABLE="TMP_F_RWD_CHOICE_CUS_WK_A"
...
then
	...
	QUALIFY TIM.DAY_KEY=MAX(TIM.DAY_KEY)
		OVER (PARTITION BY CUS_KEY,RWD_DEF_KEY,WK_KEY);
	"
	...
	print_msg "The minimum wk key is : ${MIN_WK_KEY}"
	export MIN_WK_KEY	
...
then		
	...
	FROM ${TEMP_DB}.${TEMP_TABLE} TMP
		, (
			SELECT CUS_KEY
				...
				, CUS_HIST_KEY
				, RWD_DEF_KEY
			FROM ${VIEW_DB}.V_DWH_F_RWD_CHOICE_CUS_WK_A
	...
	set_audit_log_var
	run_query -d "${TEMP_DB}" -q "${UPDATE_SQL}" -m "Unable to Update Points Wk Records in temp table" 	
	print_msg "${TEMP_TABLE} loaded successfully"
...
then
	...
	FROM ${TEMP_DB}.${TEMP_TABLE} SRC
		WHERE NOT EXISTS (
			SELECT 1
			FROM ${VIEW_DB}.V_DWH_F_RWD_CHOICE_CUS_WK_A TGT
