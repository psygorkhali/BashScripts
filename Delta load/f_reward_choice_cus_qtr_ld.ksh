...
#               table DWH_F_RWD_CHOICE_CUS_QTR_A using SOURCE TABLE 
...
TARGET_TABLE="DWH_F_RWD_CHOICE_CUS_QTR_A"
TEMP_TABLE="TMP_F_RWD_CHOICE_CUS_QTR_A"
...
####### SET CURRENT QTR KEY ######################################
print_msg "Getting CURR_QTR_KEY from DWH_D_CURR_TIM_LU"

GET_CURR_QTR_KEY_SQL="
SELECT TRIM(CURR_QTR_KEY)
FROM ${VIEW_DB}.V_DWH_D_CURR_TIM_LU;
"

CURR_QTR_KEY=$(get_result -d "${VIEW_DB}" -q "$GET_CURR_QTR_KEY_SQL" -m "Unable to get Current Quarter Key")
print_msg "The current quarter key is : ${CURR_QTR_KEY}"
export CURR_QTR_KEY
...
then
	...
	QUALIFY TIM.DAY_KEY=MAX(TIM.DAY_KEY)
		OVER (PARTITION BY CUS_KEY,RWD_DEF_KEY,QTR_KEY);
	"
	...
	####### SET MIN QTR KEY ######################################
	print_msg "Getting MIN_QTR_KEY from ${TEMP_TABLE}"
	
	GET_MIN_QTR_KEY_SQL="
	SELECT TRIM(MIN(QTR_KEY))
	FROM ${TEMP_DB}.${TEMP_TABLE};
	"
	
	MIN_QTR_KEY=$(get_result -d "${VIEW_DB}" -q "$GET_MIN_QTR_KEY_SQL" -m "Unable to get Minimum Quarter Key")
	print_msg "The minimum qtr key is : ${MIN_QTR_KEY}"
	export MIN_QTR_KEY	
...
then		
	...
	FROM ${TEMP_DB}.${TEMP_TABLE} TMP
		, (
			SELECT CUS_KEY
				...
				, CUS_HIST_KEY
				, RWD_DEF_KEY
			FROM ${VIEW_DB}.V_DWH_F_RWD_CHOICE_CUS_QTR_A
			WHERE QTR_KEY BETWEEN ${MIN_QTR_KEY}
				AND ${CURR_QTR_KEY} 
		)LU
	...
	set_audit_log_var
	run_query -d "${TEMP_DB}" -q "${UPDATE_SQL}" -m "Unable to Update Points Quarter Records in temp table" 	
	print_msg "${TEMP_TABLE} loaded successfully"
...
then
	...
	FROM ${TEMP_DB}.${TEMP_TABLE} SRC
		WHERE NOT EXISTS (
			SELECT 1
			FROM ${VIEW_DB}.V_DWH_F_RWD_CHOICE_CUS_QTR_A TGT
