...
######################################################################
# Script : f_points_cus_mth_history_ld.ksh
# Description : This script loads the Points data
#               table DWH_F_POINTS_CUS_MTH_A using SOURCE TABLE 
...
TARGET_TABLE="DWH_F_POINTS_CUS_MTH_A"
TEMP_TABLE="TMP_F_POINTS_CUS_MTH_A"
...
do
	...
	MTH_CLAUSE=$(echo ${ARR_HALF_YR_SEG[1]} | tr "." " ")
	
	print_msg "HALF YEAR CLAUSE:=${HALFYR_CLAUSE}"
	print_msg "MTH KEY CLAUSE:=${MTH_CLAUSE}"
	...
	then
		...
		QUALIFY TIM.DAY_KEY=MAX(TIM.DAY_KEY)
			OVER (PARTITION BY CUS_KEY,MTH_KEY);
		"
		...
		FROM ${TEMP_DB}.${TEMP_TABLE} TMP
			, (
				SELECT CUS_KEY
					, MTH_KEY
					, CUS_HIST_KEY
				FROM ${VIEW_DB}.DV_DWH_D_CUS_AS_OF_MTH_LU
				WHERE MTH_KEY ${MTH_CLAUSE}
			)LU
...
#	fi
	...
	then
		...
		WHERE NOT EXISTS (
			SELECT 1
			FROM ${VIEW_DB}.V_DWH_F_POINTS_CUS_MTH_A TGT
