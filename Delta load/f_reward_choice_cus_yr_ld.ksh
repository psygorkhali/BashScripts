...
#               table DWH_F_RWD_CHOICE_CUS_YR_A using SOURCE TABLE 
...
TARGET_TABLE="DWH_F_RWD_CHOICE_CUS_YR_A"
TEMP_TABLE="TMP_F_RWD_CHOICE_CUS_YR_A"
...
then
	...
	QUALIFY TIM.DAY_KEY=MAX(TIM.DAY_KEY)
		OVER (PARTITION BY CUS_KEY,RWD_DEF_KEY,YR_KEY);
	"
	...
	print_msg "The minimum yr key is : ${MIN_YR_KEY}"
	export MIN_YR_KEY	
	...
	FROM ${TEMP_DB}.${TEMP_TABLE} TMP
		, (
			SELECT CUS_KEY
				...
				, CUS_HIST_KEY
				, RWD_DEF_KEY
			FROM ${VIEW_DB}.V_DWH_F_RWD_CHOICE_CUS_YR_A
...
then
	...
	FROM ${TEMP_DB}.${TEMP_TABLE} SRC
		WHERE NOT EXISTS (
			SELECT 1
			FROM ${VIEW_DB}.V_DWH_F_RWD_CHOICE_CUS_YR_A TGT
