...
######################################################################
# Script : f_points_cus_qtr_ld.ksh
# Description : This script loads the Points data
#               table DWH_F_RWD_CHOICE_CUS_HALFYR_A using SOURCE TABLE 
...
TARGET_TABLE="DWH_F_RWD_CHOICE_CUS_HALFYR_A"
TEMP_TABLE="TMP_F_RWD_CHOICE_CUS_HALFYR_A"
...
####### SET CURRENT HALF YR KEY ######################################
print_msg "Getting CURR_HALFYR_KEY from DWH_D_CURR_TIM_LU"

GET_CURR_HALFYR_KEY_SQL="
SELECT TRIM(CURR_HALFYR_KEY)
FROM ${VIEW_DB}.V_DWH_D_CURR_TIM_LU;
"

CURR_HALFYR_KEY=$(get_result -d "${VIEW_DB}" -q "$GET_CURR_HALFYR_KEY_SQL" -m "Unable to get Current Half year Key")
print_msg "The current half year key is : ${CURR_HALFYR_KEY}"
export CURR_HALFYR_KEY
...
then
	...
	QUALIFY TIM.DAY_KEY=MAX(TIM.DAY_KEY)
		OVER (PARTITION BY CUS_KEY,RWD_DEF_KEY,HALFYR_KEY);
	"
	...
	####### SET MIN HALF YEAR KEY ######################################
	print_msg "Getting MIN_HALFYR_KEY from ${TEMP_TABLE}"
	
	GET_MIN_HALFYR_KEY_SQL="
	SELECT TRIM(MIN(HALFYR_KEY))
	FROM ${TEMP_DB}.${TEMP_TABLE};
	"
	
	MIN_HALFYR_KEY=$(get_result -d "${VIEW_DB}" -q "$GET_MIN_HALFYR_KEY_SQL" -m "Unable to get Minimum Half Year Key")
	print_msg "The minimum qtr key is : ${MIN_HALFYR_KEY}"
	export MIN_HALFYR_KEY	
...
then		
	...
	FROM ${TEMP_DB}.${TEMP_TABLE} TMP
		, (
			SELECT CUS_KEY
				...
				, CUS_HIST_KEY
				, RWD_DEF_KEY
			FROM ${VIEW_DB}.V_DWH_F_RWD_CHOICE_CUS_HALFYR_A
			WHERE HALFYR_KEY BETWEEN ${MIN_HALFYR_KEY}
				AND ${CURR_HALFYR_KEY} 
		)LU
	...
	set_audit_log_var
	run_query -d "${TEMP_DB}" -q "${UPDATE_SQL}" -m "Unable to Update Points Half year Records in temp table" 	
	print_msg "${TEMP_TABLE} loaded successfully"
...
then
	...
	FROM ${TEMP_DB}.${TEMP_TABLE} SRC
		WHERE NOT EXISTS (
			SELECT 1
			FROM ${VIEW_DB}.V_DWH_F_RWD_CHOICE_CUS_HALFYR_A TGT
